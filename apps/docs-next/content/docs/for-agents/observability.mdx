...
- `createFileTraceSink(dir)`, `buildTraceReport`, `renderTraceViewerHtml`. See [Trace viewer](/docs/recipes/trace-viewer).

### Devtools server

- `createDevtoolsServer`, `toSseFrame`. See [Devtools server](/docs/recipes/devtools-server).

### Signed audit log

- `createSignedAuditLog`, `createInMemoryAuditStore`. Hash-chain + HMAC. See [Audit log](/docs/recipes/audit-log).
