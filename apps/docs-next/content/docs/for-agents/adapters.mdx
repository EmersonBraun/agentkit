...
- `createFallbackAdapter([candidates], { shouldRetry, onFallback })`
  ...
  failures. See [Fallback chain](/docs/recipes/fallback-chain).
...
- A/B providers without users: [`speculate`](/docs/recipes/speculative-execution) or [`replayAgainst`](/docs/recipes/replay-different-model).
- Test without keys: pair `recordingAdapter` + `replayAdapter` ([deterministic replay](/docs/recipes/deterministic-replay)).
