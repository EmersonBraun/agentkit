...
1. **Record** the old session with [`createRecordingAdapter`](/docs/recipes/deterministic-replay).
2. **Tweak** the prompt, generate a new output.
3. **Snapshot** the new output with [`matchPromptSnapshot`](/docs/recipes/prompt-snapshots). If it matches — you're done.
...
- [Prompt snapshot testing](/docs/recipes/prompt-snapshots)
